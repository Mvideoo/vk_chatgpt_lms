...
class UserRepository:
    ...
    async def add_user(self, user_id: int, screen_name: str | None = None, gender: str | None = None, name: str | None = None):
        ...
        async with self.session_maker() as session:
            ...
            async with session.begin():
                user = Users(user_id=user_id, screen_name=screen_name, gender=gender, name=name)
                try:
                    session.add(user)
                except Exception:
                    return False
